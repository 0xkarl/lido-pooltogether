...
contract OneSplitMock is OneSplit {
    ...
    {
        returnAmount =
            prices[keccak256(abi.encodePacked(fromToken, destToken))] *
            amount;
    ...
    function swap(
        ...
        uint256 amountReturn =
            prices[keccak256(abi.encodePacked(fromToken, destToken))] * amount;
        ...
        if (destToken == ETH_ADDRESS) {
            ...
            require(
                IERC20(destToken).transfer(msg.sender, amountReturn),
                "erc20-send-failed"
            );
        }
    }
